...
//import io.bartholomews.fsclient.core.oauth.v2.OAuthV2.{AccessToken, RedirectUri, RefreshToken, ResponseHandler}
...
//abstract class AuthApiSpec[DE] extends WireWordSpec with ServerBehaviours[DE] {
////  implicit def accessTokenSignerResponseHandler: ResponseHandler[DE, AccessTokenSigner]
////  implicit def nonRefreshableTokenSignerResponseHandler: ResponseHandler[DE, NonRefreshableTokenSigner]
//
//  implicit def ha[T]: ResponseHandler[DE, T]
//
...
//      def request: SttpResponse[DE, NonRefreshableTokenSigner] = sampleClient.auth.clientCredentials
//
//      def endpoint: MappingBuilder = post(urlMatching("/accounts/api/token"))
//
//      behave like clientReceivingUnexpectedResponse(endpoint, request)
